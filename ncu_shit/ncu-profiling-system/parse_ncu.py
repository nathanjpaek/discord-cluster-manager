...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 parse_ncu.py <ncu_output_dir> [kernel_code_file]")
        sys.exit(1)
    ...
    parser = NCUParser(output_dir)
    parser.parse_all()
    
    # Save JSON output
    json_file = os.path.join(output_dir, "parsed_metrics.json")
    ...
    print("="*60)
    print("\nPerformance Metrics:")
    for key, value in parser.parsed_data["summary"].items():
        print(f"  {key}: {value:.1f}%")
    
    print("\nIdentified Bottlenecks:")
    for bottleneck in parser.parsed_data["bottlenecks"]:
        print(f"  - {bottleneck.get('type', 'Unknown')} (Severity: {bottleneck.get('severity', 'unknown')})")
    
    print("\nGenerated Recommendations:", len(parser.parsed_data["recommendations"]))
    print("\nFiles created:")
    print(f"  - {json_file}")
    print(f"  - {prompt_file}")
